...
`dubstudio` is a Go CLI that converts a subtitle file (`.srt` or `.vtt`) into a WAV audio track using ElevenLabs Text-to-Speech, matching the subtitle timings — for replacing a video's audio track using only its subtitles as the script.
